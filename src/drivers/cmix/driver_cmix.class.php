...
class Driver_cmix {
     ...
     * @var string
     */
    private $_config = 'drivers/cmix/configuration.json';
    ...
    public function can($data){
        ...
        if (!is_array($data) || count($data) == 0 || !is_string($data[0])) {
            Restos::log(Restos::LOG_DEBUG, 'driver_cmix (can): Capability can not empty and must be a string');
            return false;
        }

        if (!property_exists($this->_capabilities, $data[0])) {
            Restos::log(Restos::LOG_DEBUG, 'driver_cmix (can): Capability ' . $data[0] . ' does not exists');
            return false;
        }
        ...
        if (count($data) > 1) {
            ...
            //If is used the syntax: can('capability', Driver_cmix::CONTEXT_APP, $id) //$id can be empty or null
            if (is_int($data[1]) && count($data) == 2) {
                ...
                Restos::log(Restos::LOG_DEBUG, 'driver_cmix (can): Bad parameters number (' . count($data[0]) . ') or types. Capability: ' . $data[0]);
                return false;
            }
        }
