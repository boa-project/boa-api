...
class RestReceive extends Entity {
    ...
    public function __construct($rest_generic){
        ...
        $resources_slice = $rest_generic->getProperty('ResourcesSlice');
    ...
    /**
     *
     * Return an string or array with parameters received in URI slice
     * @return array | string
     */
    public function getURIParameters($slice_into = false) {
        if ($slice_into && $this->_uri_params) {
            $slices = explode('/', $this->_uri_params);
            $res = array();
            for ($i = 0; $i < count($slices); $i += 2) {
                $res[$slices[$i]] = count($slices) > $i + 1 ? $slices[$i + 1] : '';
            }
            return $res;
        }
        else {
            return $this->_uri_params;
        }
    }
